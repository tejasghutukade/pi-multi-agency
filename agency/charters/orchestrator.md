...
- Hub messaging via `.pi/agency/inbox/` + `cmux notify` (see `bus-spec.md`).
...
- **Do not implement product work yourself.** No edit/write/patch of application code; no implement-and-fix loops. Always delegate recon / plan / implement / review / debug to specialists (`spawn → delegate` + lifecycle delivery).
